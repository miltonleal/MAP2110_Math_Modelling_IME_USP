...
def Solução_Iterativo(V, L, C, constante_c):
    ...
    while abs(Erro) >= 1e-5:
        ...
        for i in range(len(Z_k1)):
            norma_1_diferenca += (abs(Z_k1[i] - Z_k[i]))
            #print("norma da diferença =", norma_1_diferenca)
        print ("norma da diferença", norma_1_diferenca)
...
def main():
    ...
    # Vetores V L C
    Solução_Escalonamento = Encontrando_X(numero_nodes, Matriz_Escalonada)
    print("\nvetor solução do modo escalonamento", Solução_Escalonamento)
    print("\nSoma solução escalonamento: ", sum(Solução_Escalonamento))
    ...
    # Solução Iterativa
    Solução_Iterativa = Solução_Iterativo(V, L, C, constante_c)
    print("\nvetor solução do modo iterativo", Solução_Iterativa)
    print("\nSoma solução interativa: ", sum(Solução_Iterativa))

    ### DIFERENÇA entre os modos

    diferença = []
    for k in range (len(Solução_Escalonamento)):
        diferença.append(abs(Solução_Escalonamento[k]-Solução_Iterativa[k]))
